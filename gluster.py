...
class Translator:
	def __init__ (self, c_this):
		# This is only here to keep references to the stubs we create, because
		# ctypes doesn't and glupy.so can't because it doesn't get a pointer to
		# the actual Python object. It's a dictionary instead of a list in case
		# we ever allow changing fops/cbks after initialization and need to
		# look them up.
		self.stub_refs = {}
		...
		if "lookup_fop" in funcs:
			...
			def stub (frame, this, loc, xdata, s=self):
				s.lookup_fop(frame,this,loc,xdata)
				return 0
			self.stub_refs["lookup_fop"] = stub
			dl.set_lookup_fop(c_this,stub)
		if "lookup_cbk" in funcs:
			@OpLookup.cbk_type
			def stub (frame, cookie, this, op_ret, op_errno, inode, buf, xdata,
					  postparent, s=self):
				s.lookup_cbk(frame,cookie,this,op_ret,op_errno,inode,buf,xdata,
							 postparent)
				return 0
		...
		if "create_fop" in funcs:
			@OpCreate.fop_type
			def stub (frame, this, loc, flags, mode, umask, fd, xdata, s=self):
				s.create_fop(frame,this,loc,flags,mode,umask,fd,xdata)
				return 0
			self.stub_refs["create_fop"] = stub
			dl.set_create_fop(c_this,stub)
		if "create_cbk" in funcs:
			@OpCreate.cbk_type
			def stub (frame, cookie, this, op_ret, op_errno, fd, inode, buf,
					  preparent, postparent, xdata, s=self):
				s.create_cbk(frame,cookie,this,op_ret,op_errno,fd,inode,buf,
